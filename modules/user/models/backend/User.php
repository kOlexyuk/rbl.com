...
class User extends \app\modules\user\models\User
{
    ...
    public function scenarios()
    {
        ...
        $scenarios[self::SCENARIO_ADMIN_CREATE] = ['username', 'email', 'status', 'role', 'newPassword', 'newPasswordRepeat','phone'];
        $scenarios[self::SCENARIO_ADMIN_UPDATE] = ['username', 'email', 'status', 'role', 'newPassword', 'newPasswordRepeat','phone'];
        return $scenarios;
    }
    ...
    public  function ChangeEmailBlockUser(){
        $this->email = time().'-'.$this->email;
        $this->phone = $this->phone."-".time();
