 ...
 */
class ServiceAreaSearch extends Model
{
    public $id;
    public $name;
    public $name_ru;
    ...
    {
        $query = (new \yii\db\Query())->from('service_area');
        ...
        $query->leftJoin('service_area_lang',"service_area.id = service_area_lang.id  and service_area_lang.lang =':lang'",[':lang'=>'ru']); //Yii::$app->language
...
//       $query->select(['service.id', 'coalesce(service_lang.name,service.name) as name', 'coalesce(service_lang.note,service.note) as note']);
        $query->select(['service_area.id', 'service_area.name',  'service_area_lang.name as name_ru'
